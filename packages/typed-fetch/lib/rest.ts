import type { TypedRequestInit, TypedResponseInit } from "./schema.ts";

export class TypedRequest <T extends TypedRequestInit> extends Request {
...
export const serializeResponseObject = (init: TypedResponseInit): Response => {

	const body = init ? JSON.stringify(init) : null;
	const headers = new Headers(init.headers);

	if (init) headers.set('content-type', 'application/json');

	return new Response(body, { headers, status: init.status });
};
